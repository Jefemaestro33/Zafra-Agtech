...
**Versión 0.5** — 21 de marzo de 2026
Guadalajara, Jalisco
...
- Deploy en Railway con URL pública: https://agtech-sistema-production.up.railway.app/ — ✓ LISTO
- Dockerfile multi-stage (Node build dashboard + Python serve API + estáticos) — ✓ LISTO
- Repo GitHub: Jefemaestro33/agtech-sistema (privado) con auto-deploy en cada push a main — ✓ LISTO
...
- Hardware y laboratorio pendientes de financiamiento (junio 2026)
...
| v0.5 | 21 mar 2026 | Deploy en producción en Railway (URL pública), Dockerfile multi-stage (Node + Python), repo GitHub privado con auto-deploy, proyecto reorganizado (.gitignore, .env.example, README.md, requirements.txt) |
...
| API REST | FastAPI (api.py) | 20 endpoints para dashboard + integraciones. CORS, docs automáticos en /docs. Sirve estáticos del dashboard en producción | ✓ LISTO — deployado en Railway |
...
| Código | GitHub | Repo privado Jefemaestro33/agtech-sistema, auto-deploy a Railway en cada push a main | ✓ LISTO (v0.5) |
...
|----------|---------|--------|-------|
| GitHub | Repo privado Jefemaestro33/agtech-sistema | ✓ LISTO (v0.5) | Gratis |
| Railway PostgreSQL | 8 tablas, 416K lecturas + 2,160 clima + 560 microbioma | ✓ LISTO | Gratis (trial $5 USD) |
| Railway App (API + Dashboard) | Dockerfile multi-stage, auto-deploy desde GitHub | ✓ LISTO (v0.5) | Gratis (trial) |
| URL pública | https://agtech-sistema-production.up.railway.app/ | ✓ LISTO (v0.5) | — |
...
| 5 | Crear repo en GitHub | ✓ LISTO (v0.5) — Jefemaestro33/agtech-sistema |
...
| 18 | Deploy backend + dashboard en Railway como URL pública | ✓ LISTO (v0.5) — https://agtech-sistema-production.up.railway.app/ |
...
### 17.4 Producción — ✓ DEPLOYADO (v0.5)

**URL:** https://agtech-sistema-production.up.railway.app/

**Deploy:** Dockerfile multi-stage en Railway, auto-deploy desde GitHub en cada push a main.
- Stage 1 (Node 20 Alpine): `npm ci` + `npm run build` → genera `dashboard/dist/`
- Stage 2 (Python 3.13 slim): `pip install` + copia backend/ + copia dist/ → `uvicorn api:app`
- FastAPI sirve API en `/api/*` y estáticos del dashboard en `/` (SPA fallback a index.html)
- Un solo servicio Railway = API + dashboard, sin CORS, mismo dominio
...
AgTech Sistema v0.5 — Deployado en producción en Railway. API + dashboard en un solo servicio. Auto-deploy desde GitHub.
